...
from colossalai.nn.optimizer import HybridAdam,Lamb
...
def train(args) -> None:
    ...
    if args.lora_rank > 0:
        # lora_config = LoraConfig(task_type="CAUSAL_LM", target_modules=['q_proj','v_proj'],r=args.lora_rank, lora_alpha=32, lora_dropout=0.1)
        lora_config = LoraConfig(target_modules=['q_proj','v_proj'],r=args.lora_rank, lora_alpha=32, lora_dropout=0.1)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if isinstance(plugin, HybridParallelPlugin) and plugin.pp_size > 1:
            ...
            total_loss = torch.tensor(0.0, device=f'npu:{get_current_device()}' if isinstance(get_current_device(), int) else get_current_device())
            for step, batch in enumerate(dataloader, start=start_step):
                batch = {k: v.to(f'npu:{DEVICE_ID}') for k, v in batch.items() if isinstance(v, torch.Tensor)}
