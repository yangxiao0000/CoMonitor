...
def supervised_tokenize_sft(
    ...
    # )
    turn_lengths = [
    len(tokenizer([template.get_prompt(2 * x)], add_special_tokens=False)["input_ids"][0])
    for x in turns
    ]

    # 使用 bisect_right 查找目标索引
    target_turn_index = bisect.bisect_right(turn_lengths, max_length - 1)
